...
show_public_ip() {
    ...
    echo "  - Shadowsocks 可直接使用 IP Address"
...
setup_cloudflare_ddns() {
    ...
    while [[ -z "$cf_domain" ]]; do
        ...
        if [[ -z "$cf_domain" ]]; then
            warn "Domain cannot be empty，请重新输入"
        fi
    done
...
setup_duckdns() {
    ...
    while [[ -z "$duck_domain" ]]; do
        ...
        if [[ -z "$duck_domain" ]]; then
            warn "子Domain cannot be empty，请重新输入"
        fi
    done
...
install_vmess() {
    ...
    if [[ "$DOMAIN" == "__IP_DIRECT__" ]]; then
        ...
        DOMAIN="$SERVER_IP"
        log "IP direct mode, server IP: $SERVER_IP"
        warn "VMess+WS+TLS recommended with domain，IP直连将使用自签名证书"
    fi
    ...
    if [[ $IP_MODE -eq 0 ]]; then
        # 有域名模式 - 使用acme.sh申请证书
        log "正在Applying SSL certificate..."
...
show_subscription() {
    ...
    echo -e "${CYAN}----------------------------------------${NC}"
    echo ""
    echo -e "${GREEN}Online subscriptionAddress:${NC}"
    echo ""
...
show_banner() {
    echo ""
    echo -e "${CYAN}============================================================${NC}"
    echo -e "${GREEN}           VPS Toolbox - One-Click Deploy Tool v2.4.0${NC}"
    echo -e "${CYAN}============================================================${NC}"
